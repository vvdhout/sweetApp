...
contract ERC721Token is ERC721 {
    ...
    // Who owns this token (address)?
    mapping(uint256 => address) tokenToOwner; 
    // How many tokens does this address own?
    mapping(address => uint256) ownerToBalance; 
    // Which address is approved to handle this token?
    mapping(uint256 => address) tokenToApproved;
    // Which addresses are set as operators for an address?
    mapping(address => mapping(address => bool)) ownerToOperator;
    
    // Modifier that checks if the caller (msg.sender) is the tokenOwner or if he is approved
    ...
    function mint(uint256 _tokenId) internal { 
        // Make sure the token is not owned by somebody else
        require(tokenToOwner[_tokenId] == address(0), "this token belongs to someone else already");
        
        // Set the msg.sender as token owner
        tokenToOwner[_tokenId] = msg.sender; 
        // Increase token balance of msg.sender by 1
        ownerToBalance[msg.sender] += 1;
        
        // Emit token Transfer event from non-existing address to msg.sender
